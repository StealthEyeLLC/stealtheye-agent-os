...
export const RuntimeLiveCapabilityStatusSchema = z.enum(["disabled", "fixture_only", "read_only_live"]);
export const RuntimeToolDescriptorSchema = z.object({schema_version:z.literal(McpRuntimeSchemaVersion.RuntimeToolDescriptor),name:z.string(),description:z.string(),inputSchema:z.record(z.unknown()),outputSchema:z.record(z.unknown()),annotations:z.object({readOnlyHint:z.literal(true),destructiveHint:z.literal(false),openWorldHint:z.literal(false),idempotentHint:z.literal(true)}),riskTier:z.string(),safety:z.object({readOnly:z.literal(true),previewOnly:z.boolean(),fixtureOnly:z.boolean(),destructive:z.literal(false),live:z.boolean(),liveWrite:z.literal(false).default(false),externalSideEffect:z.literal(false).default(false),liveCapabilityStatus:RuntimeLiveCapabilityStatusSchema,allowedEffects:z.array(z.string()),hardStopCategories:z.array(z.string()),receiptExpectations:z.array(z.string()),evidenceExpectations:z.array(z.string())}),mcp_compatible:z.literal(true),app_host_tool:z.boolean(),repo_status_tool:z.boolean().default(false),memory_graph_tool:z.boolean().default(false)});
...
export const RuntimeToolCallResultSchema = z.object({schema_version:z.literal(McpRuntimeSchemaVersion.RuntimeToolCallResult),tool_name:z.string(),ok:z.boolean(),structuredContent:z.unknown(),content:z.array(z.object({type:z.literal("text"),text:z.string()})),receipt:z.unknown(),_meta:z.object({runtime_id:z.string(),read_only:z.literal(true),preview_only:z.boolean(),fixture_only:z.boolean(),live_write:z.literal(false).default(false),external_side_effect:z.literal(false).default(false),live_capability_status:RuntimeLiveCapabilityStatusSchema})});
