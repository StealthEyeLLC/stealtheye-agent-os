...
export function createLocalDevTransport(server = createMcpRuntimeServer()) {
  return { local_dev_only: true, bind_default: "127.0.0.1", routes: ["GET /health", "GET /capabilities", "GET /metadata", "GET /dev-mode", "POST /tools/call", "GET /resources", "POST /resources/read"], async handle(request: RuntimeHttpLikeRequest): Promise<RuntimeHttpLikeResponse> {
    ...
    if (request.method === "POST" && request.path === "/tools/call") { const body = request.body as { name?: string; input?: unknown }; if (!body?.name) return { status: 400, json: { ok: false, error: "missing tool name" } }; return { status: 200, json: await server.callTool(body.name, body.input ?? {}) }; }
